...
- 自动扫描 `/userdisk/Novels/` 目录下的 `.txt` 小说文件
...
```
/userdisk/Novels/三体.txt
/userdisk/Novels/斗破苍穹.txt
/userdisk/Novels/凡人修仙传.txt
```

### 手动输入书名

手动输入书名时，不需要输入完整路径。比如小说文件是 `/userdisk/Novels/三体.txt`，在阅读器里输入 `三体` 或 `三体.txt` 即可打开。
...
上传后的小说会自动保存到 `/userdisk/Novels/` 目录。
...
例如文件路径是 `/userdisk/Novels/三体.txt`，输入 `三体` 或 `三体.txt` 都可以。
